...
async function assertCanAct(user, id, verb = "الاعتماد", expectStage = null) {
  ...
  // ⚠️ الطلب المنتهي يُردّ برسالةٍ تقول ما جرى: كان يمرّ إلى أودو فيرفضه
  // بـ«لا يمكن اعتماد طلب في حالته الحالية» (خطأ ٥٠٠ غامض)، أو يُردّ بـ«تحرّك
  // الطلب» — وكلاهما لا يخبر المعتمِد أن اعتماده نجح أصلًا. فكان يعيد الضغط
  // ويظنّ التطبيق معلّقًا: وقع فعلًا في HR-REQ-00397، ضُغط الزرّ إحدى عشرة مرّة
  // بعد اعتمادٍ ناجح صدر به خطابه.
  const FINAL = { done: "اكتمل", rejected: "مرفوض", cancelled: "ملغى" };
  if (FINAL[rec.state])
    throw badRequest(`هذا الطلب ${FINAL[rec.state]} بالفعل — لا يحتاج ${verb} مرّةً أخرى.`);
