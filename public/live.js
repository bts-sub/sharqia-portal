  ...
  function pollNotifs() {
    ...
    fetch("/api/notifications", { credentials: "include", cache: "no-store" })
      ...
      .then(function (j) {
        ...
        writeSeen(maxId);
        // ⚠️ لا تُعاد الصفحة لأجل إشعار: التطبيق يجلب بياناته كل ثلاثين
        //   ثانية فيظهر الإشعار وحده، وإعادةُ التحميل تقطع على الموظف عمله
        //   وتبدو له «تعليقًا». يُكتفى بتنبيه النظام.
        systemNotice(fresh);
      })
      .catch(function () {});
  }
